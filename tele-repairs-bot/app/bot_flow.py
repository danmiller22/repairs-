...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "PAID_BY":
        ...
        await update.message.reply_text("Choose who paid.", reply_markup=reply_kb(PAIDBY_CHOICES))
        return
    ...
    if state == "CONFIRM":
        # Ignore text; require inline buttons
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state != "INVOICE":
        return  # ignore photos outside invoice step
    ...
    buf.seek(0)
    drive = DriveClient()
    link = drive.upload_invoice_bytes(name=f"invoice_{update.effective_user.id}_{int(datetime.utcnow().timestamp())}.jpg", content=buf.read(), mimetype="image/jpeg")
    form = context.user_data.get("form", {})
    form["InvoiceLink"] = link or ""
...
async def ask_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Send a photo of the invoice, or Skip.", reply_markup=ReplyKeyboardMarkup([[SKIP, BACK, CANCEL]], resize_keyboard=True))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cancel_inline":
        ...
        return
    if data == "edit":
        # Simple implementation: jump to the first field again
        context.user_data["state"] = "DATE"
